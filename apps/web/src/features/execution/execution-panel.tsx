...
import { ArrowLeft, ArrowRight, Check, CirclePause, CirclePlay, Clock3, ListChecks, RotateCcw } from "lucide-react";
...
export function ExecutionPanel({ run, onRunChanged }: { run: ExperimentRun; onRunChanged: (run: ExperimentRun) => void }) {
  ...
  if (["draft", "cancelled", "archived", "completed"].includes(current.run.status)) {
  ...
  return <section aria-labelledby="execution-heading" className={styles.execution}>
    <Card className={styles.executionHeader}><div><span className={styles.eyebrow}>Live execution</span><h2 id="execution-heading">{paused ? "Experiment paused" : "Experiment in progress"}</h2><p>{completedCount} of {current.steps.length} steps completed</p></div><div className={styles.runControls}><Badge tone={paused ? "warning" : "success"}>{paused ? "Paused" : "Running"}</Badge>{paused ? <Button disabled={working} onClick={() => void mutate(() => resumeRunExecution(run.id, current.run.revision), "Experiment resumed")}><RotateCcw aria-hidden="true" size={18} />Resume</Button> : <Button disabled={working} variant="secondary" onClick={() => void mutate(() => pauseRunExecution(run.id, current.run.revision), "Experiment paused")}><CirclePause aria-hidden="true" size={18} />Pause</Button>}</div><div aria-label={`${progress}% complete`} className={styles.progressTrack} role="progressbar" aria-valuemax={100} aria-valuemin={0} aria-valuenow={progress}><span style={{ width: `${progress}%` }} /></div></Card>
